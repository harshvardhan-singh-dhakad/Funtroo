...
const CATEGORIES = [
  { label: '✨ View All',       value: '',         href: '/shop' },
  { label: '💜 For Her',        value: 'for-her',    href: '/sex-toys-for-women' },
  { label: '⚡ For Him',        value: 'for-him',    href: '/sex-toys-for-men' },
  { label: '💑 Couples',        value: 'couples',    href: '/couples-sex-toys' },
  { label: '💧 Lubricants',     value: 'lubricants', href: '/lubricants' },
  { label: '👙 Lingerie',       value: 'lingerie',   href: '/lingerie' },
]
...
export default function ShopContent({ 
  ...
  const category = baseCategory || sp.get('category') || ''
  ...
  function setParam(key: string, val: string) {
    ...
    if (key === 'category') {
      ...
      if (catObj && catObj.href) {
        router.push(catObj.href)
        return
      }
    }
  ...
  const displayTitle = h1Title || (category ? currentCategoryLabel : 'All Products')

  return (
    <div className="flex-1 max-w-6xl mx-auto px-3 sm:px-4 py-4 md:py-8 w-full">

      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 md:mb-6">
        <div>
          <h1 className="font-display text-2xl md:text-3xl text-f-dark">{displayTitle}</h1>
          <p className="text-f-gray text-xs md:text-sm mt-0.5 mb-2">{total} products available</p>
          {introText && <p className="text-sm text-f-gray max-w-2xl">{introText}</p>}
        </div>
        ...
        <div className="flex-1 min-w-0">
          {loading ? (
            ...
            <div className="text-center py-16 bg-white border border-f-border rounded-2xl p-6">
              ...
              <Link 
                href="/shop" 
                className="inline-block mt-4 px-4 py-2 bg-f-purple text-white text-xs rounded-xl hover:bg-f-mid transition"
              >
                Reset All Filters
              </Link>
            </div>
      ...
      {PAGE_FAQS[faqKey as keyof typeof PAGE_FAQS] && (
        <section className="mt-12 bg-white border border-f-border rounded-2xl px-4 py-8">
          <FAQSection faqs={PAGE_FAQS[faqKey as keyof typeof PAGE_FAQS]} title={`${displayTitle} Frequently Asked Questions`} />
        </section>
      )}
    </div>
  )
